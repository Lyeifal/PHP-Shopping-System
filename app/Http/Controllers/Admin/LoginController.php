...
class LoginController extends CommonController
{
    ...
    /**
     * 处理登录表单数据
     * @param Acct_admin $user 管理员表模型
     * @throws \myframe\HttpException 通过HttpException异常来返回数据
     */
    public function login(Acct_admin $user)
    {
        //获取用户名和密码
        ...
        //根据提交的密码计算加密后的密码
        $psMD5 = $this->passwordMD5($password, $data['paa_salt']);
//        var_dump($data['paa_password']);
//        var_dump($psMD5);
        //判断密码是否正确
        if ($data['paa_password'] != $psMD5) {
            //返回结果
            $this->error('密码错误');
        }
        //保存登录态
